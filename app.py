...
st.markdown("""
    <div style='text-align: left; font-size: 22px; padding-top: 10px; color: #FFFFFF;'>
        Select a county and see the forecasted EV adoption trend for the max 5 years.
    </div>
""", unsafe_allow_html=True)
...
future_rows = []
forecast_horizon = st.slider("Forecast Horizon (months)", min_value=12, max_value=60, value=36)
...
# Create the forecast period text for the chart title
if forecast_horizon >= 12:
    years = forecast_horizon / 12
    if years.is_integer():
        title_period = f"{int(years)} {'Year' if years == 1 else 'Years'}"
    else:
        title_period = f"{years:.1f} Years"
else:
    title_period = f"{forecast_horizon} Months"

# Use the dynamic period in the chart title
ax.set_title(f"Cumulative EV Trend - {county} ({title_period} Forecast)", fontsize=14, color='white')
...
if historical_total > 0:
    ...
    forecast_period = f"{forecast_horizon} months"
    # Convert to years if appropriate
    if forecast_horizon >= 12:
        years = forecast_horizon / 12
        if years.is_integer():
            forecast_period = f"{int(years)} {'year' if years == 1 else 'years'}"
        else:
            forecast_period = f"{years:.1f} years"
    st.success(f"Based on the graph, EV adoption in **{county}** is expected to show a **{trend} of {forecast_growth_pct:.2f}%** over the next {forecast_period}.")
else:
...
multi_counties = st.multiselect("Select up to 4 counties to compare", county_list, max_selections=4)
...
if multi_counties:
    ...
    ax.set_title(f"EV Adoption Trends: Historical + {title_period} Forecast", fontsize=16, color='white')
    ax.set_xlabel("Date", color='white')
    ax.set_ylabel("Cumulative EV Count", color='white')
    ax.grid(True, alpha=0.3)
    ax.set_facecolor("#1c1c1c")
    ...
    st.success(f"Forecasted EV adoption growth over next {title_period} — {growth_sentence}")
